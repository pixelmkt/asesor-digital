...
// ── WIDGET ──
async function loadWidgetConfig(){const c=await api('/api/config');if(!c?.widget)return;const w=c.widget;document.getElementById('w-name').value=w.name||'';document.getElementById('w-avatar').value=w.avatar||'';document.getElementById('w-primary').value=w.primaryColor||'#D4502A';document.getElementById('w-secondary').value=w.secondaryColor||'#1E1E1E';document.getElementById('w-bg').value=w.bgColor||'#ffffff';document.getElementById('w-text').value=w.textColor||'#2C2C2C';document.getElementById('w-position').value=w.position||'right';document.getElementById('w-mode').value=w.mode||'floating';document.getElementById('w-greeting').value=w.greeting||'';document.getElementById('w-chips').value=(w.chips||[]).join('\n');document.getElementById('w-header').value=w.headerTitle||'';const fabEl=document.getElementById('w-fab-icon');if(fabEl){fabEl.value=w.fabIcon||'';if(w.fabIcon)previewFabIcon(w.fabIcon);}const url=c.backend_url||location.origin;document.getElementById('embed-code').textContent=`<script src="${url}/widget.js"><\/script>`;}
async function saveWidgetConfig(){const fabEl=document.getElementById('w-fab-icon');const data={name:document.getElementById('w-name').value,avatar:document.getElementById('w-avatar').value,fabIcon:fabEl?fabEl.value:'',primaryColor:document.getElementById('w-primary').value,secondaryColor:document.getElementById('w-secondary').value,bgColor:document.getElementById('w-bg').value,textColor:document.getElementById('w-text').value,position:document.getElementById('w-position').value,mode:document.getElementById('w-mode').value,greeting:document.getElementById('w-greeting').value,chips:document.getElementById('w-chips').value.split('\n').filter(Boolean),headerTitle:document.getElementById('w-header').value};const r=await api('/api/config/widget',{method:'PUT',body:JSON.stringify(data)});toast(r?.shopifySaved===false?'Widget guardado (⚠️ sin sync Shopify)':'Widget guardado ✓','ok');}
function previewFabIcon(url){const img=document.getElementById('fab-preview');if(!img)return;if(url){img.src=url;img.style.display='block';}else{img.style.display='none';}}
async function loadShopifyFilesForFab(){const container=document.getElementById('shopify-files-fab');if(!container)return;container.innerHTML='<div style="font-size:12px;color:var(--mut);">Cargando archivos Shopify...</div>';container.style.display='flex';const r=await api('/api/shopify/files?type=image');if(!r?.files?.length){container.innerHTML='<div style="font-size:12px;color:var(--mut);">No se encontraron imágenes. Sube un GIF en tu Shopify Admin → Contenido → Archivos</div>';return;}container.innerHTML=r.files.map(f=>`<div onclick="setFabIcon('${f.url}')" style="cursor:pointer;border:2px solid transparent;border-radius:8px;padding:3px;transition:border-color .15s;" onmouseover="this.style.borderColor='var(--red)'" onmouseout="this.style.borderColor='transparent'"><img src="${f.url}" style="width:60px;height:60px;border-radius:50%;object-fit:cover;" title="${f.filename||''}"><div style="font-size:9px;color:var(--mut);text-align:center;margin-top:3px;max-width:60px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${(f.filename||'').replace(/^.*\//,'')}</div></div>`).join('');}
function setFabIcon(url){const el=document.getElementById('w-fab-icon');if(el)el.value=url;previewFabIcon(url);toast('GIF seleccionado — guarda el widget para aplicar','ok');}
